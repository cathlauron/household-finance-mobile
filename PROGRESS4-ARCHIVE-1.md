# PROGRESS4-ARCHIVE-1.md — Archived detailed session logs

This file holds older, detailed dated "Session entry" write-ups that
were moved out of PROGRESS4.md to keep it shorter to paste each
session. Nothing here is deleted — everything from these sessions that
still matters (fixes, decisions, known issues, file changes) is already
folded into PROGRESS4.md's own ✅ Done / 📌 Decisions / ⚠️ Known issues /
📁 Files / ▶️ Next step sections. This file is a historical record only.

Covers: the original 13-bug on-device fix pass, the first on-device
re-test pass, the design-change implementation sessions (Android date
picker, bottom nav Calendar removal, PIN toggle, SUB/CANCELLED badge,
"which of these is you?" confirm step, Reports checkbox redesign,
ToPay/Planning pill reversal, Transactions swipe-to-navigate on
Bills/Debts/Loans/Income/Savings-derived rows), the "fewer words" pass
through LoansScreen/IncomeScreen/CreateProfileScreen, B.12b-1
(Pension/SS offset), and B.12b's original scoping session. PROGRESS4.md
picks up from B.12b-2 onward.
...
### Session — LoansScreen.tsx "fewer words" pass complete (18 items)
...
### Session — IncomeScreen.tsx "fewer words" pass complete (14 items)
...
### Session — CreateProfileScreen.tsx "fewer words" pass complete (19 items)

Trimmed all 19 flagged items: 4 Firebase error-translation messages (email
already in use, invalid email, weak password, generic fallback), 3 local
validation errors (invalid email, password length, username taken), 3
recovery-key alert bodies (cloud-save failure, save success, still-offline
retry), 2 occurrences of a "Continue to App" button (the alert action and
the modal's primary button — same text, two distinct real spots, trimmed to
"Continue" in both), 1 generic profile-save failure error, 1 screen
subtitle, 1 confirm-password placeholder, 1 encryption hint below the form,
1 modal title, 1 modal subtitle, 1 copy-key button label, 1 checkbox

...
- Automated Maestro test for Change Password (create profile -> change password -> sign out -> sign in with new password -> revert to original), full pass.
 testIDs added to password fields, sign-out button, and Home/Settings tabs. (A.7.9)
...
- **Four items explicitly parked, not scheduled into the numbered sequence, pending a dedicated decision session:**
  ...
  - Android home screen widget / app shortcut for quick-logging a transaction without opening the app (inspired by TapSheet's iOS Shortcuts integration) — requires migrating off Expo Go to a custom EAS development build, a real one-time infrastructure change. Deliberately deferred to Phase C, since C.1 already requires setting up EAS Build for real installable app releases — natural to fold this in at that point rather than doing the migration twice.
...
- **Issue C — CONFIRMED FIXED via live retest.** The fix implemented and deployed in the earlier session (cancelLinkCode() called from startHouseholdLink()/startHouseholdInvite(), the simplified linkCodes delete rule, and the isFinished flag on subscribeToLinkCode) was retested live and passed: generating a new invite code correctly invalidates the old one, and an old code no longer successfully links a joiner. Closed.
...
**Account Recovery (Secret Recovery Key + Household Peer Recovery) is fully done and confirmed working — no further action needed.** Both features built, an accidental full wipe of the work was caught and cleanly restored, Firestore rules deployed live, and both the solo and linked recovery paths were confirmed via real hands-on testing. Nothing further planned here unless a new gap is found in normal use.
...
4. **Checkpoint A.7 — Auth/Linking/Security fixes & features** (inserted before Phase B, closes out the linking/auth work before UI/UX polish begins):
  ...
  | ~~A.7.5~~ | ~~60-second cooldown before generating a new code~~ | ✅ Done | Cooldown attached to the "Code expired? Start over with a new code" link (not the initial "Start linking" button, which becomes unreachable once a code exists) — disabled + greyed while active, with a live "Generate a new code in Xs" countdown ticking every second via a dedicated setInterval, separate from the existing 15-min expiry timer and never cleared by expiry itself. Bundled UX fix: added a "Generating your secure code…" message next to the spinner on the initial code-generation button, after investigating a "feels slow" report and confirming it was a reassurance/feedback gap (no real performance issue — PBKDF2/encryption/Firestore write legitimately take a few seconds, connection was solid, delay was never measured). Verified on-device: full 5-step test (initial generation shows feedback text, cooldown shows on Start Over and counts down, reaches 0 and re-enables, Start Over correctly restarts cooldown, leaving/returning to screen mid-cooldown doesn't crash) — all passed. |
   ...
   | ~~A.7.6b~~ | ~~Extend join flow + keep-mine/keep-theirs/merge choice to households with 2+ existing members~~ | ✅ Confirmed done, live-tested | Committed (`8f1e5f7`). |
   | ~~A.7.6c~~ | ~~UI: show "X of 5 linked" + block/explain once cap is hit~~ | ✅ Confirmed done, live-tested | Committed (`0fdc963`). |
   | ~~A.7.6d~~ | ~~Owner-only UI: remove members, ownership transfer, auto-dissolve~~ | ✅ Confirmed done, live-tested | Committed. Full member roster, owner-only remove button, successor picker on owner-leaves, and shared auto-dissolve logic all built together as one screen in `SettingsScreen.tsx`. See ✅ Done for full detail. |
   | ~~A.7.7~~ | ~~Show profiles of everyone you're linked to~~ | ✅ Confirmed done, live-tested | Delivered as part of A.7.6d's household roster (name, "You" label, "Owner" label) — no separate build needed. |
...
- mobile-app/src/screens/SettingsScreen.tsx — UPDATED (Account Recovery, this session): Household section shows/approves a pending peer-recovery request (discovered via `pendingRecoveryRequestId` on the household doc, per Correction 1); Security section gained a retroactive "Secret Recovery Key" setup/view card for profiles created before this feature existed.
- mobile-app/src/household.ts — UPDATED (Account Recovery, this session): added `pendingRecoveryRequestId` field support on the household document.
- mobile-app/src/DataContext.tsx — UPDATED (Account Recovery, this session): exposes `getHouseholdKey`/`getHouseholdId` so the approver's device can access the in-memory household key during peer approval.
- mobile-app/firestore.rules — UPDATED (Account Recovery, this session) and DEPLOYED LIVE via `firebase deploy --only firestore:rules`: owner-only access rules for `recoveryKeys/{username}`; member-only read/create/approve/delete rules for `householdRecovery/{requestId}`; household members permitted to update `pendingRecoveryRequestId`.
...
**What happened:** Investigated whether resetting a Firebase Auth password could recover access to encrypted data (it can't — Firebase Auth and the local data-encryption passphrase are currently the same typed value with no server-side path to the encryption key). Scoped and built two zero-knowledge recovery features via the Claude+Antigravity investigate-then-approve workflow: **Feature A (Secret Recovery Key)** — a one-time recovery code generated at profile creation (or retroactively for existing profiles) that wraps a copy of the data key, stored in a new `recoveryKeys/{username}` Firestore collection; **Feature B (Household Peer Recovery)** — a linked member can request recovery from another linked, signed-in member, who approves via a 6-digit code and hands over the shared household key through a one-time ephemeral transfer key. Two corrections were applied to the implementation plan before approval: pending-request discovery moved onto the `households/{householdId}` document (an open-ended Firestore query isn't something security rules can support) instead of a broken `list` query, and the existing "Clear all data & start fresh" Danger Zone button was reused for the both-lost dead-end case instead of building a duplicate wipe feature. Implemented, verified via `npx tsc --noEmit` and a full Android bundle export, committed and pushed (`7cfe26f`).

**Then it vanished.** Testing found none of the new screens/cards appeared anywhere, even after a completely fresh app+Firebase state. Investigation traced it to VS Code's Local History: about 2 minutes after `7cfe26f` was pushed, an editor "workspace edit" flush silently wrote stale, unsaved tab contents for the same 7 files back to disk, reverting all of them (including truncating `recovery.ts` to 0 bytes). The next session's routine sync-check checkpoint commit then faithfully captured and pushed that reversion as `abf86f1` — deleting the feature from `origin/main` without anyone touching git directly. `git reflog` confirmed no `reset`/`checkout`/`stash` was involved; this was purely an editor-vs-disk race, not a git mistake.

**Restored cleanly:** `git revert --no-edit abf86f1` (commit `7b6e989`) brought all 7 files back to an exact, 0-difference match with `7cfe26f` — `recovery.ts` confirmed back to its full 332 lines. Firestore rules were re-deployed live via `firebase deploy --only firestore:rules` (confirmed "Deploy complete!"), and both `npx tsc --noEmit` and an Android bundle export were re-confirmed clean.

**Tested for real, both paths passed:** generated a Secret Recovery Key on an existing profile via Settings > Security. For the linked path, created two throwaway linked test profiles and manufactured the locked-out state via Firebase Console's own "Reset password" action (confirmed this only changes the Firebase Auth side, not the local encryption passphrase — and confirmed separately that the in-app "Change password" flow keeps both in sync by design, so it had to be avoided for this test) — the full request → 6-digit-code approval → automatic unlock handshake worked end-to-end.

**New standing habit adopted because of this incident:** close or Save All any open VS Code tabs for files about to be edited, before starting a session — see 📌 Decisions made.

🧹 Code health
- `npx tsc --noEmit`: clean, confirmed multiple times (original implementation, and again after the restore).
- Android bundle export (`npx expo export --platform android`): clean, confirmed both times.
- Firestore rules deployed live twice (original implementation, and again after the restore) — both confirmed via `firebase deploy --only firestore:rules` success output.
- Committed and pushed: `7cfe26f` (original), `abf86f1` (accidental revert, from an automated checkpoint), `7b6e989` (fix — reverts `abf86f1`). `git status` clean after the final push.

▶️ Next step (this entry only — see the top-of-file ▶️ Next step section for current status)
- Account Recovery is fully done and confirmed working. No further action needed unless a new gap surfaces in normal use.
- Going forward: close/Save All open editor tabs before any session involving file edits, to prevent this specific failure mode from recurring.

## 📅 Session entry — Full A.7.6b/c/d + A.7.7 batch live test CONFIRMED PASSING — Checkpoint A.7 fully closed, Phase A complete

**What happened:** Ran the deferred batch live two-phone test covering A.7.6b (join-choice 
dialog for 2+ member households), A.7.6c ("X of 5 linked" badge + cap enforcement), A.7.6d 
(owner-only remove-member, ownership transfer on owner-leave, auto-dissolve on last member), 
and A.7.7 (member roster display). Everything passed, including the specific scenario the 
live Firestore listener was built for — one phone leaving/being removed while the other has 
the app open in the foreground, confirming the dissolve happens live rather than only on 
next app reopen.

Combined with the earlier-confirmed Issues A, B, and C (household linking real-time updates 
and stale-code invalidation), this closes out every remaining item in Checkpoint A.7. 
**Phase A (Firebase Auth) is now fully complete.**

No code changes this session — test-result log entry only.
...
▶️ Next step
- Phase A is done. Decide next: pick up the parked multi-device "active sessions" feature 
  (design already scoped — see 📌 Decisions made), or move straight into Phase B (UI/UX 
  Polish). Revisit with the person to decide which is the better use of the next session.

...
### Session — First full on-device testing pass (real results; reminder/notification testing deferred to Phase C)
- Ran the full accumulated on-device testing checklist for the first time — everything across Sections 1–14 of the testing doc that had been sitting as "code-complete, `npx tsc --noEmit` clean, pending on-device verification," covering B.7 through B.14, both bug-audit sweeps, the Loans/saveModel sweep, the full swipe-to-delete rollout, and all of the B2.1/B2.3 iconization batches.
- Decision (person confirmed): all testing and any bugs specific to reminders/notifications are DEFERRED to Phase C rather than chased now, since real push-notification delivery and deep-linking are much more reliable to verify on a real installed build (EAS/TestFlight, once C.1 happens) than through Expo Go. This covers Section 2.2 (reminder tap → To-Pay deep-link), Section 2.3 (Bill reminders/Weekly recap toggle independence — toggle wasn't even visible in Settings), Section 3.2 (duplicate subscription notification), all of Section 8 (Weekly Spending Recap notification), and all of Section 11 (Subscription Cancel-Reminder, including its notification wording and deep-linking). See the new "🔔 Deferred to Phase C" list under ⚠️ Known issues for the full breakdown, so nothing gets lost between now and Phase C.
- Everything else — real bugs and design-change requests found outside the reminder/notification area — is logged fresh below under ⚠️ Known issues, in a new "🐛 First on-device testing pass — real results" block that supersedes the old "code-complete, pending on-device verification" placeholders for anything actually covered this pass. A few items genuinely couldn't be tested (no legacy profile on hand, single-device household) and are noted as such rather than assumed fine.
- Headline findings: several fixes previously logged as ✅ CODE-COMPLETE in earlier sessions do NOT appear to actually work on-device — the Left to Spend caution threshold doesn't persist, the Emergency Fund "Saved" checkmark doesn't show, offline deletes hang instead of showing an error, the 3 background-save warnings never appeared, the Travel checklist-delete expense cleanup didn't happen, and the FI Calculator's new fields (SWR/return/savings/Years Until FI/save) barely function at all. These need a fresh investigation pass before being trusted again — `npx tsc --noEmit` clean only confirms the code compiles, not that the logic is reachable/correct at runtime.
- Also found one new must-fix outside any previously-tracked item: after signing out while offline and reconnecting, sign-in fails with both "Incorrect password" and "missing information" shown at once, recovering only after a full app + Expo Go restart and re-scanning the QR code.
- Several items are explicit design-change requests rather than bugs (Reports screen redesigned as a checkbox-driven show/hide list; ToPay/Planning tabs to show icon+title instead of icon-only; bottom nav to drop Calendar in favor of a tappable date element on Home; a custom-themed calendar popup instead of Android's native date picker; a toggle instead of a "Turn Off" button for PIN; a confirm/save step before "this is me" takes effect; swipe-to-delete enabled on Transactions' derived rows with a source-aware confirmation) — captured as their own items below, to be scoped separately from straightforward bug fixes.
- No code changes made this session — testing-and-logging only. Next session works through the real bugs one at a time via the standard Antigravity-investigates-first workflow.
...
🐛 First on-device testing pass — real results (supersedes the "on-device testing deferred" placeholders below for anything actually covered here)

🔔 Deferred to Phase C — reminder & notification testing (not to be chased now)
- Section 2.2 — tapping a subscription reminder notification switching to To-Pay → Bills → the specific bill. Not tested this pass (needs a real fired reminder); deferred.
- Section 2.3 — "Bill reminders" and "Weekly recap" independent toggles in Settings → Notifications. Tester could not find this option in Settings at all. Deferred — needs its own investigation once Phase C's real-notification testing environment is in place, rather than debugging blind through Expo Go now.
- Section 3.2 — subscription bills not double-notifying. Not tested (needs waiting for a real due date); deferred.
- Section 8 (all) — Weekly Spending Recap: toggle/day-pills/hour input, turning it off, and the actual notification firing with a matching amount. Not tested this pass; deferred in full.
- Section 11 (all) — Subscription Cancel-Reminder: the reminder notification wording, warm-app tap-to-deep-link, and (the hardest case) cold-start tap-to-deep-link. Deferred in full. Two cosmetic issues surfaced while looking at this feature (the subscription checkbox showing 2 checkmarks instead of 1, the SUB/CANCELLED badge not visually reading as a badge, and the "Keep" button showing no visible feedback when tapped) are NOT notification-timing issues and are tracked separately below rather than deferred, since they're plain UI bugs independent of when/whether a reminder fires.

🐞 Real bugs confirmed this pass (need investigation + fix)
- **Offline sign-out lockout is back, in a new shape.** After turning on Airplane Mode, signing out worked, but signing back in after turning Airplane Mode back off failed with BOTH "Incorrect password" and "missing information" shown at the same time. Recovery only worked after fully closing the app and Expo Go and re-scanning the QR code — a full session reset, not just retrying the password. Needs its own investigation — the earlier bug-audit fix (#2, "offline sign-out lockout") only addressed sign-OUT while offline, not sign-IN immediately after reconnecting.
- **Offline delete hangs instead of showing an error (Accounts/Bills/Debts).** With Airplane Mode on, deleting a record got stuck in a loading state indefinitely — no error message appeared. The delete only completed once Airplane Mode was turned back off. Contradicts the earlier "6 lower-priority audit follow-ups" fix (3.1), which was supposed to show a visible error on a failed offline delete — needs re-investigation; the failure mode is a stuck spinner, not a silent no-op, so the existing catch block may not be wired to the right loading-state flag.
- **Left to Spend caution threshold doesn't persist.** Changing it from 20 to 35 in Settings and fully closing/reopening the app reverts it back to 20. Contradicts the original B.7 build note that this saves on blur like the "notify me X days before" field.
- **Emergency Fund "Saved" checkmark still doesn't appear.** The second-sweep bug-audit fix (#5) was supposed to make the EF save button show a checkmark + "Saved" matching the FI Calculator button beside it — on-device, it still shows no confirmation at all.
- **Background-save warnings never show.** None of the 3 promised `Alert.alert(...)` warnings (failed password-change cloud backup, failed account-recovery re-save, failed household-unlink personal backup) appeared during testing, and the tester notes "background saving does not work" more generally. Needs a fresh investigation — this was previously logged as fixed and `npx tsc --noEmit` clean.
- **Events/Goals/Groceries/Settings saveModel try/catch sweep fails silently in practice.** The dedicated session that added try/catch + error UI across ~20 save/delete call sites in these 4 files does not appear to be surfacing errors on-device — tester reports failures still happen silently. Likely the same root cause as the offline-delete hang above (a loading/busy flag not resolving on failure, masking the error UI underneath).
- **Travel checklist-item delete still leaves its expense in Transactions.** Bug-audit fix #3 (deleting, not unchecking, a checked/costed Travel checklist item should remove its linked expense) does not appear to actually take effect on-device.
- **FI Calculator (Savings tab) is largely non-functional on-device.** "Years Until FI" only ever displays the literal placeholder text "years until fi," never a real computed duration. The show/hide-projected-date toggle from the B.12a build notes does not appear anywhere on screen. Saving the new fields (SWR, expected return, monthly savings, current savings) does not persist at all — not even locally on the same device. Needs a full re-investigation of B.12a's actual on-device state.
- **Biometric unlock didn't work at all on a Face ID device with fingerprint off.** Rather than just a wrong label, the tester reports Face ID "was not able to capture it" — may be a functional failure, not only the cosmetic label-matching fix from the last session. Needs investigation separate from the text-label fix already shipped.
- **Turning PIN off feels frozen — no loading indicator.** After confirming the removal action, the screen shows no spinner or other feedback while it's being removed, reading as if the app had hung, even though it eventually completes.
- **Category Watchlist "over budget" wording fires one step too early.** At exactly 100% of the limit it already shows "over the budget," but 100% is still AT the limit, not over it — should only appear once spend genuinely exceeds 100%.
- **"Which of these is you?" picker doesn't update Transactions live.** Selecting a different person immediately updates the picker itself, but Transactions kept showing the old owner until the app was fully closed and reopened — should reflect immediately.
- **AccountsScreen Cards/List toggle — "Stacked card view" label didn't appear.** Tapping the Cards icon correctly switched the view, but the expected brief floating label (B2.1's IconLabelHint) never showed. The List-view label wasn't separately confirmed either way, so re-check both.

🎨 Design-change requests surfaced during testing (not bugs — need scoping/prioritizing, not a quick fix)
- **Android date picker** — replace the native Android date picker with the app's own themed calendar popup, so it doesn't look visually detached from the rest of the app (Section 1.7). Previously an accepted platform limitation; person now wants it addressed properly.
- **PIN "Turn Off" control** — replace the text "Turn Off" button with a toggle switch instead (Section 1.2).
- **SUB/CANCELLED badge redesign** — should render as a real hollow-box badge, matching how Accounts' Cash/Debit/Credit badges look, instead of blending in beside the bill's title as plain text (Section 11.1 — cosmetic part only, not the deferred reminder-timing part).
- **"Which of these is you?" needs a confirm/save step** — currently a single instant tap with no confirmation, risking a misclick; person wants an explicit save/confirm step added.
- **Bottom nav — drop Calendar as a bottom tab.** Person wants Calendar removed from the 5-icon bottom bar entirely, replaced by a small date element at the top-center of the Home screen that navigates to Calendar when tapped.
- **Swipe-to-delete on Transactions' derived (non-manual) rows.** Currently only manual rows can be swiped; person wants derived rows (bill/debt/loan/income/savings-sourced) to also support swipe-to-delete, paired with either (a) a warning that deleting will also remove it at its source, or (b) redirecting to the source screen to confirm the deletion there — exact approach still needs deciding.
- **Reports screen redesign.** Person wants the 9 report icons (B2.3 batch 1) replaced with a dropdown-style list of icon+title rows, each with a checkbox on the left — checking a report makes it appear in the Reports screen's normal tab row, unchecking hides it, to cut down on clutter. This supersedes the ReportsScreen batch-1 iconization work with a new interaction model.
- **ToPayScreen/PlanningScreen tabs — icon+title, not icon-only.** Person wants these two screens' segmented pills (batch 2) to show both an icon and its label at all times, reversing the icon-only decision made for these two screens specifically.

✅ Confirmed working as designed this pass
- "Copy Recovery Key" in Settings → Security copies successfully (Section 1.3).
- Bills/Loans/Debts summary-line ordering matches across all three screens (Section 1.5) — no issue reported.
- Lock screen "Use password instead" flow (Section 1.6) — no issue reported; treated as working.
- Section 6.5's known accepted gap (solo profile has no "which of these is you?" picker) was independently confirmed — the tester's own profile doesn't show the picker at all, consistent with the documented gap rather than a new bug.

⏸️ Could not be tested this pass (not a pass/fail — needs the right conditions)
- Section 2.4 (legacy-profile crash guards on Home/Dashboard) — no legacy profile available to test with.
- Two-device checks throughout (Left to Spend sync, Category Watchlist, Yours/Mine/Ours cross-device labeling, FI Calculator sync) — single-device household, not testable yet.
...
▶️ Next step
- Work through the real bugs found in the first on-device testing pass, one at a time, via the standard Antigravity-investigates-first workflow — see "🐛 First on-device testing pass — real results" under ⚠️ Known issues above for the full list. Suggested rough priority, since several of these were previously believed already fixed and now need re-diagnosis: (1) the new offline sign-out→sign-in lockout (blocks actual usage); (2) offline delete hanging instead of erroring (Accounts/Bills/Debts); (3) Left to Spend threshold not persisting; (4) the FI Calculator's largely-nonfunctional new fields; (5) Emergency Fund "Saved" checkmark; (6) background-save warnings; (7) Events/Goals/Groceries/Settings saveModel silent failures; (8) Travel checklist-delete expense cleanup; (9) the remaining smaller bugs (biometric capture, PIN-off loading indicator, Category Watchlist's 100% threshold wording, "which of these is you?" not updating Transactions live, AccountsScreen's missing "Stacked card view" label).
- Reminder/notification testing and any bugs found in that area (subscription reminder deep-linking, Bill reminders/Weekly recap toggle independence, duplicate-notification suppression, Weekly Spending Recap firing) are explicitly DEFERRED to Phase C — do not schedule investigation time on these until C.1 (EAS Build) is done and the app is running as a real installed build rather than through Expo Go. See the "🔔 Deferred to Phase C" list above so nothing gets lost in the meantime.
- Once the person is ready, separately scope and prioritize the design-change requests surfaced this pass (Android date picker replacement, PIN toggle instead of "Turn Off" button, SUB badge redesign, "which of these is you?" confirm step, bottom-nav Calendar removal + Home date element, Transactions swipe-to-delete on derived rows, Reports screen checkbox-driven redesign, ToPay/Planning icon+title reversal) — these are new work, not bug fixes, and can be sequenced independently of the bug list above.

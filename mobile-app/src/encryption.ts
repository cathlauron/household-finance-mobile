...
// How many times the passphrase is "stretched" before it becomes the real encryption key.
...
// A random string, unique per profile, mixed into the passphrase before deriving a key —
// so two people with the same passphrase still get two different encryption keys.
...
// Turns a typed passphrase + that profile's salt into the real encryption key.
export function deriveKey(passphrase: string, saltHex: string): CryptoJS.lib.WordArray {
...
// Reverses encryptJSON. Throws if the key is wrong (e.g. incorrect passphrase) — callers
// should catch this and show "incorrect passphrase" rather than let the app crash.

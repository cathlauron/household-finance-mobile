...
- 3.1 — Month grid view. ✅ Complete.
  - New file: mobile-app/src/screens/CalendarScreen.tsx — shows a full month grid (Sun–Sat columns), with today's date highlighted with a gold circle border.
  - Prev (‹) / Next (›) arrow buttons flip between months, wrapping correctly across year boundaries (e.g. December → January rolls the year forward).
  - "Today" pill button jumps straight back to the current month regardless of how far you've navigated.
  - Uses the same useTheme() color system from Phase 2.2, so it already respects light/dark mode with no extra work.
  - mobile-app/src/navigation/MainTabs.tsx updated: the "Calendar" tab now points at CalendarScreen instead of PlaceholderScreen. Every other tab is untouched and still shows PlaceholderScreen.
  - Confirmed working on a real Android phone via Expo Go: grid renders correctly, today is circled, month navigation works both directions.

🔧 In progress

Nothing in progress right now — Phase 3.1 is complete. Ready to start Phase 3, Checkpoint 3.2 — Tap a day to see what's happening that day (a simple popup).
...
- Dev workflow in Codespaces: Because this project runs in GitHub Codespaces (cloud-based, not on the person's home network), Metro must always be started with tunnel mode, and — important, learned this session — it must be run from INSIDE the mobile-app folder, not the repo root:
    cd mobile-app
    npx expo start --tunnel
  Running `npx expo start` from the repo root fails with "ConfigError: The expected package.json path ... does not exist" because package.json lives inside mobile-app/, not at the top level. Always `cd mobile-app` first in a fresh terminal session before starting Metro. The plain `npx expo start` (without --tunnel) also still risks starting Metro on a local network address the phone can't reach — always confirm the terminal shows an address ending in .exp.direct before scanning the QR code.
...
- Calendar month math (Checkpoint 3.1): Standard plain-JavaScript Date math (no calendar library added) — new Date(year, month + 1, 0).getDate() gets the number of days in a month, and new Date(year, month, 1).getDay() gets which day of the week the 1st falls on, to figure out how many empty cells to pad the grid with at the start. Kept intentionally simple since more advanced calendar features (event dots, tap-to-see-day-details, running balance) are separate later checkpoints (3.2, 3.3) that will build on top of this same file rather than needing a rewrite.
- Checkpoint tracking discipline: Learned twice now (Phase 1→2 handoff, and again during Phase 2.2) that work completed in one session didn't make it into PROGRESS.md accurately before the next session started. Both times were caught safely via git history rather than causing real damage, but it's a reminder to ALWAYS run the full "wrap up this session" step (including a fresh, complete PROGRESS.md rewrite) before ending a session, even if it feels like a small change.
- File-creation discipline: Pasting multi-line code directly at the bash `$` prompt fails silently/loudly (bash tries to run it as commands) rather than saving it. Files are always created either via a `cat > filename << 'ENDOFFILE' ... ENDOFFILE` block (content included inside the same paste) or by opening the file in the Codespace's built-in editor — never by pasting code at a bare prompt.
- Overwrite-safety discipline: Before creating any file with a `cat > filename << 'ENDOFFILE'` block, ALWAYS first check whether that file already exists (e.g. via `cat filename`) rather than assuming a fresh file is needed. This was followed correctly this session — CalendarScreen.tsx was checked as not-yet-existing before being created.
...
- Session-start verification discipline (NEW, learned this session): A GitHub webpage fetch showed a stale/cached snapshot of the repo (only 2 commits, missing files) that contradicted the person's own terminal output twice in a row. This was resolved by trusting the person's own git evidence (origin/main matching HEAD exactly, a full sensible commit history) over the webpage fetch, once it became clear the webpage snapshot was unreliable rather than the git evidence being wrong. Going forward: git status + git log from the person's own terminal is the primary source of truth; a supplementary GitHub webpage check is a nice-to-have, not something to block progress on if it disagrees with solid git evidence.

▶️ Next step

Phase 3 — Calendar (M5), Checkpoint 3.2 — Tap a day to see what's happening that day. This adds a simple popup/modal when a day cell in the grid is tapped, showing what's scheduled for that day (matching the web app's Calendar day-click behavior — see household-finance-app-spec-and-scale.md §8 and the .html reference file's Calendar day-modal logic for what should eventually show there, though the actual bill/debt/income data wiring comes in later checkpoints — for 3.2 itself, a simple placeholder popup confirming which day was tapped is enough).
...
- mobile-app/ folder (Expo project — built and saved directly via the Codespace terminal, not the GitHub website)
  ...
  - mobile-app/src/screens/CalendarScreen.tsx — real month grid calendar, with prev/next navigation and today highlighted
